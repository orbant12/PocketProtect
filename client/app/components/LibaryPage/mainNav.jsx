...
export function Horizontal_Navbar({
    ...
    return(
        <ScrollView horizontal style={[{paddingTop:65,paddingLeft:25,flexDirection:"row",backgroundColor:"white",paddingBottom:15,borderWidth:0.3,width:"100%"},{position:"absolute",zIndex:30 }]} showsHorizontalScrollIndicator="false">
            {options.map((data) => (
                <TouchableOpacity onPress={() => {setIsSelected(data.value);scrollViewRef != undefined && scrollViewRef.current.scrollTo(data.scroll)}} style={isSelected == data.value ? {borderBottomColor:"magenta",borderBottomWidth:2,marginRight:30} : {marginRight:30}}>
